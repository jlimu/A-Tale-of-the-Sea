...
#include <fstream>
#include "zonas.h"
const int NumZonas = 49;


class map {
public:
	map(std::string usuario, objetos *object, enemigos *enemigo,int filas, int columnas);
	map() { _mapa[NumZonas] = 0; }
	void mostarMapa(const int &x,const int &y);
	void mostarZona(const int &pos) { _zona[pos].mostrarZona(); }
